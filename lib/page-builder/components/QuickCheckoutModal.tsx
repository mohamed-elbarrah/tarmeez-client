...
export const QuickCheckoutModal = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-lg p-0 overflow-hidden rounded-2xl border-none">
        {orderCode ? (
          <div className="flex flex-col items-center gap-6 p-10 text-center bg-white">
            ...
            <div className="flex flex-col gap-2">
              <h2 className="text-2xl font-bold text-[var(--h-color)]">شكراً لك! تم استلام طلبك</h2>
              <p className="text-[var(--t-color)] opacity-70">رقم الطلب الخاص بك هو:</p>
              <span className="text-2xl font-mono font-bold text-[var(--p-color)]">{orderCode}</span>
            </div>
            <p className="text-sm text-[var(--t-color)] opacity-60">سيقوم فريقنا بالتواصل معك قريباً لتأكيد الطلب</p>
            <button
              onClick={() => {
                setOrderCode(null);
                onClose();
          ...
          <div className="p-6 flex flex-col gap-8 bg-white">
            <DialogHeader>
              <DialogTitle className="text-right text-xl font-bold text-[var(--h-color)]">إتمام الطلب السريع</DialogTitle>
            </DialogHeader>
            ...
            <div className="flex items-center gap-4 p-4 bg-[var(--color-bg,#f8fafc)] rounded-2xl">
              ...
              <div className="flex flex-col text-right">
                <span className="font-bold text-[var(--h-color)]">{product.name}</span>
            ...
            {/* Checkout Form */}
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 text-right" dir="rtl">
              <div className="flex flex-col gap-1">
                <label className="text-sm font-bold text-[var(--h-color)]">الاسم الكامل</label>
                <input
              ...
              <div className="flex flex-col gap-1">
                <label className="text-sm font-bold text-[var(--h-color)]">رقم الهاتف</label>
                <input
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-sm font-bold text-[var(--h-color)]">المدينة</label>
                  <input
                    {...register('city')}
                    className={cn(
                      "p-3 rounded-xl bg-white border border-[var(--p-color)]/20 focus:outline-none focus:border-[var(--p-color)]",
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-sm font-bold text-[var(--h-color)]">الحي / الشارع</label>
                  <input
                    {...register('street')}
                    className={cn(
                      "p-3 rounded-xl bg-white border border-[var(--p-color)]/20 focus:outline-none focus:border-[var(--p-color)]",
              ...
              {(errors.city || errors.street) && (
                <span className="text-xs text-red-500">يرجى إكمال تفاصيل العنوان</span>
              )}
