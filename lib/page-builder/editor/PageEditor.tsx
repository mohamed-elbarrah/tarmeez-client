...
import {
  ...
  type PageBuilderStoreData,
} from "@/lib/page-builder/context/StoreDataContext";
...
export default function PageEditor({ page: initialPage }: PageEditorProps) {
  ...
  const [updateStatus, { isLoading: isStatusChanging }] =
    useUpdatePageStatusMutation();

  const saveTimerRef = useRef<NodeJS.Timeout | null>(null);
  const [currentPuckData, setCurrentPuckData] = useState(
    page.content?.puckData || { content: [], root: { props: {} } },
  );

  // Puck state management
  const initialData = page.content?.puckData || {
    content: [],
    root: { props: {} },
  };

  // Live store data for the Puck canvas preview
  const { data: categoriesData } = useGetCategoriesQuery();
  const { data: productsData } = useGetProductsQuery("ACTIVE");

  const storeContextValue = useMemo<PageBuilderStoreData>(
    () => ({
    ...
    [categoriesData, productsData, page.storeSlug],
  );

  const handleSave = useCallback(
    async (currentData: any, isAutoSave = false) => {
  ...
  const onPublishToggle = async () => {
    try {
      ...
      toast.success(
        newStatus === "PUBLISHED" ? "تم نشر الصفحة" : "تم إلغاء نشر الصفحة",
      );
    } catch (error: any) {
  ...
  const onPreview = () => {
    window.open(
      `/store/${page.storeSlug}/p/${page.slug}?preview=true`,
      "_blank",
    );
  };
  ...
  return (
    <div className="h-screen flex flex-col bg-background" dir="rtl">
      {/* Editor Toolbar */}
      <header className="h-16 border-b border-border bg-card px-2 flex items-center justify-between z-50">
                ...
                <div className="space-y-4 border-b pb-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(v) =>
                        updateLocalSettings("showHeader", v)
                      }
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(v) =>
                        updateLocalSettings("showFooter", v)
                      }
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      value={page.seoTitle || ""}
                      onChange={(e) =>
                        updateLocalSettings("seoTitle", e.target.value)
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        updateLocalSettings("seoDescription", e.target.value)
                      }
      ...
      <AlertDialog open={showExitDialog} onOpenChange={setShowExitDialog}>
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              لقد أجريت تغييرات لم يتم حفظها بعد. هل تريد حفظها قبل المغادرة أم
              المغادرة بدون حفظ؟
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <div className="flex-1 overflow-hidden puck-editor-container">
        <PageBuilderStoreContext.Provider value={storeContextValue}>
          <Puck
...
function Badge({
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${styles[variant || "default"]} ${className}`}
    >
      {children}
    </span>
  );
