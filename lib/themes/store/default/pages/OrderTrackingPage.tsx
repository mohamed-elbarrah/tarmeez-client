...
export default function OrderTrackingPage({ theme, storeSlug }: Props) {
  ...
  const { data, isLoading, refetch } = useTrackOrderQuery({ orderCode: code, storeSlug }, { skip: !code })
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4">
      ...
      <div className="flex gap-2 mb-6">
        ...
        />
        <button onClick={() => refetch()} className="px-8 py-4 bg-[var(--p-color)] text-white rounded-2xl font-bold hover:shadow-lg transition-all">تتبع</button>
      </div>

      {isLoading && <div className="text-center py-10 text-gray-400 font-bold">جاري البحث...</div>}
      ...
      {data && (
        <div className="bg-white p-8 border rounded-3xl shadow-sm space-y-6">
          <div className="flex justify-between items-center border-b pb-6">
            ...
            <div className="bg-blue-50 text-blue-600 px-4 py-1 rounded-full font-black text-sm">{data.status}</div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <h4 className="text-gray-400 font-bold text-xs uppercase tracking-widest">تفاصيل العميل</h4>
              <div className="font-bold">{data.customerName}</div>
              <div className="text-gray-500">{data.customerPhone}</div>
            </div>
            <div className="space-y-4">
              <h4 className="text-gray-400 font-bold text-xs uppercase tracking-widest">عنوان التوصيل</h4>
          ...
          <div className="pt-6 border-t flex justify-between items-center">
            ...
            <span className="text-2xl font-black text-[var(--p-color)]">{data.total.toLocaleString()} ر.س</span>
          </div>
        </div>
      )}
