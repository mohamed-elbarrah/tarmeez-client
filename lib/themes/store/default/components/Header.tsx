...
export default function Header({ storeName, logo, theme, cartCount, searchQuery, onSearchChange, onNavigate }: Props) {
  ...
  const handleAccountClick = () => {
    // If on root, go to /login or /account.
    // Otherwise append /login or /account to the current pathname so store pages become /store/<slug>/login
    const base = pathname && pathname !== '/' ? pathname.replace(/\/?$/, '') : ''
    if (user && user.role === 'CUSTOMER') {
      onNavigate('account')
      return
    }
    if (base) router.push(`${base}/login`)
    ...
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex items-center justify-between gap-8">
        ...
        <div className="flex items-center gap-6">
          <div onClick={handleAccountClick} className="cursor-pointer text-gray-700 flex flex-col items-center hover:text-[var(--p-color)] transition-colors">
