  ...
  return (
    <section className="relative overflow-hidden rounded-[var(--radius)] min-h-[400px] flex items-center p-8 md:p-16 text-white" style={{ backgroundColor: theme.secondary }}>
      <div className="relative z-10 max-w-lg space-y-6">
        ...
        <h2 className="text-4xl md:text-5xl font-black leading-tight">{featuredProduct.name}</h2>
        <p className="text-lg opacity-80 leading-relaxed">اكتشف أفضل العروض والمنتجات الحصرية في متجرنا اليوم. جودة نضمنها لك في كل عملية شراء.</p>
        <Link 
          href={`/store/${storeSlug}/product/${featuredProduct.slug || featuredProduct.id}`} 
          className="bg-white text-black px-10 py-3 rounded-full font-bold shadow-lg hover:scale-105 transition-transform inline-block"
        >
          تسوق الآن
        </Link>
      </div>
