...
export default function LoginPage() {
  ...
  const invitationAccepted =
    searchParams.get("message") === "invitation-accepted";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (status === 401) {
        ...
        setError(
          (data && data.message) || err?.error || "حدث خطأ أثناء تسجيل الدخول",
        );
      }
    }
  };
        ...
        <CardContent className="space-y-4">
          {invitationAccepted && (
            <Alert className="border-green-500/50 bg-green-500/10 text-green-700 dark:text-green-400">
              ...
              <AlertDescription>
                {storeName
                  ? `تم قبول دعوتك في متجر "${storeName}" بنجاح! سجّل الدخول للوصول إليه.`
                  : "تم قبول الدعوة بنجاح! سجّل الدخول للوصول إلى متجرك."}
              </AlertDescription>
            </Alert>
          )}
          {error && (
