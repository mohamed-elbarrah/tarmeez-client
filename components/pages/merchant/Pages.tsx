...
export default function PagesDashboard() {
  ...
  const handleStatusToggle = async (id: string, currentStatus: string) => {
    try {
      ...
      toast.success(nextStatus === 'DRAFT' ? 'تم استعادة الصفحة' : 'تم أرشفة الصفحة');
    } catch (error) {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold mb-2">صفحاتي</h1>
          <p className="text-muted-foreground">إدارة محتوى متجرك وصفحات الهبوط</p>
        </div>
        <Button 
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-muted/50 border-b border-border">
                <th className="text-right py-4 px-6 text-sm font-medium text-muted-foreground">العنوان</th>
                <th className="text-right py-4 px-6 text-sm font-medium text-muted-foreground">النوع</th>
                <th className="text-right py-4 px-6 text-sm font-medium text-muted-foreground">الحالة</th>
                <th className="text-right py-4 px-6 text-sm font-medium text-muted-foreground">تاريخ الإنشاء</th>
                <th className="text-right py-4 px-6 text-sm font-medium text-muted-foreground">إجراءات</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <tr key={i} className="border-b border-border animate-pulse">
                    <td colSpan={5} className="py-8 px-6"><div className="h-4 bg-muted rounded w-full"></div></td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={5} className="py-20 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <Button variant="link" onClick={() => setIsCreateModalOpen(true)}>إنشاء أول صفحة</Button>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={page.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                    <td className="py-4 px-6 font-medium">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-muted-foreground font-normal">/{page.slug}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${typeMap[page.type]?.color}`}>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-2">
                        ...
                        <Link href={`/store/${page.storeId}/p/${page.slug}`} target="_blank">
                        ...
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          title={page.status === 'ARCHIVED' ? 'استعادة' : 'أرشفة'}
                          onClick={() => handleStatusToggle(page.id, page.status)}
                        >
                          {page.status === 'ARCHIVED' ? <RefreshCcw className="w-4 h-4" /> : <Archive className="w-4 h-4" />}
                        </Button>

                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive hover:bg-destructive/10" title="حذف">
                          ...
                          <AlertDialogContent dir="rtl">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                سيتم حذف الصفحة وإزالتها من المتجر نهائياً. لا يمكن التراجع عن هذا الإجراء.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
