...
export function CheckoutFieldsEditor({
  ...
  onChange,
  disabled,
}: CheckoutFieldsEditorProps) {
  const [editingField, setEditingField] = useState<CheckoutFieldConfig | null>(
    null,
  );
  ...
  const toggleEnabled = (id: string, val: boolean) => {
    commit(
      fields.map((f) =>
        f.id === id
          ? { ...f, enabled: val, required: val ? f.required : false }
          : f,
      ),
    );
  };
      ...
      {fields.map((field, index) => {
        ...
        return (
          ...
          >
            ...
            <span
              className={cn(
                "flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border shrink-0",
                meta.color,
              )}
            >
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <div
                className={cn(
                  ...
                  !field.enabled && "opacity-30 pointer-events-none",
                )}
              >
                <span className="text-[9px] text-muted-foreground tracking-wide uppercase">
                  إلزامي
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Select
                value={addForm.type}
                onValueChange={(v) =>
                  setAddForm((f) => ({ ...f, type: v as CheckoutFieldType }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setAddForm((f) => ({ ...f, placeholder: e.target.value }))
                }
